...
export function resolveRosterResourceLimits(roster: Roster, context: BattleScribeCatalogueContext): EffectiveResourceLimits {
  ...
  const resources = [...byId].map(([typeId, definitions]): EffectiveResourceLimit => {
    ...
    return { typeId, definitions, authored, effective, provenance: overrides.length ? "player" : "source", ...(override === undefined ? {} : { override }) };
  });
...
function sourceLimit(definition: CostType): ResourceLimitState {
  ...
  const state = numericLimit(value);
  if (state.kind === "finite" && (value === 0 || definition.hidden === true ||
      (definition.node.attributes.hidden !== undefined && definition.hidden === undefined))) {
    return unresolved("Source default activation is unverified for zero or creation-hidden resources.");
  }
  return state;
}
...
function hasDynamicLimit(definition: CostType): boolean {
  ...
  const visit = (node: CostType["node"]): boolean => {
    for (const child of node.children) {
      ...
      if (node.name !== "costType" && !allowed.has(child.name)) return true;
